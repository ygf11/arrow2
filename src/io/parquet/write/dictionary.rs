...
use crate::array::{Array, DictionaryArray, DictionaryKey, PrimitiveArray, Utf8Array};
    ...
    PrimitiveArray<K>: std::fmt::Display,
{
    /*if array.values().null_count() > 0 {
        return Err(ArrowError::NotYetImplemented(
            "Writing dictionary arrays to parquet does not support non-null values".to_string(),
        ));
    }*/
    ...
    match encoding {
        Encoding::PlainDictionary | Encoding::RleDictionary => {
            ...
            let dict_page = match array.values().data_type() {
                ...
                DataType::Int32 => dyn_prim!(i32, i32, array),
                DataType::Int64 => dyn_prim!(i64, i64, array),
                DataType::Utf8 => {
                    let values = array
                        .values()
                        .as_any()
                    ...
                    let mut buffer = vec![];
                    utf8_encode_plain(values, false, &mut buffer);
                    CompressedPage::Dict(CompressedDictPage::new(buffer, values.len()))
                }
                other => {
